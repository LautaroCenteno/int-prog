...
def max_apariciones(n: int, lista: list[int]) -> int:
    ...
    """
    # máximo es el candidato a máxima cantidad de apariciones de la subsecuencia mas larga encontrada hasta el momento
    maximo:int = 0

    # contador es la cantidad de apariciones de la subsecuencia actual
    contador:int = 0
    for elem in lista:
        # si el elemento es igual al que estamos buscando, incrementamos el contador
        if elem == n:
            contador += 1
            # si el contador es mayor al maximo encontrado hasta el momento, lo actualizamos
            if contador > maximo:
                maximo = contador

        else:
            # si el elemento no es igual al que estamos buscando reiniciamos el contador
            contador = 0
    return maximo
